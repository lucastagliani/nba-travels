  ...
  return (
    <div className="flex flex-wrap gap-1">
      {analysis.daysPerCity.map(({ city, days }) => (
        <span
          key={city}
          className="rounded bg-muted px-1.5 py-0.5 text-[10px] text-fg-soft"
        >
          {city} {days}d
        </span>
      ))}
  ...
  return (
    <div className="flex flex-wrap gap-1">
      ...
      {analysis.gamesPerTeam.length > 5 && (
        <span className="text-[10px] text-subtle">+{analysis.gamesPerTeam.length - 5}</span>
      )}
    </div>
  )
}
...
export function CompareView({ options, onSelect }: CompareViewProps) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-semibold text-fg">Compare all itineraries</h2>
      ...
      <div className="hidden overflow-x-auto rounded-xl border border-border md:block">
        <table className="w-full min-w-[960px] text-left text-xs">
          <thead>
            <tr className="border-b border-border bg-card/80 text-subtle">
              ...
              return (
                ...
                >
                  <td className="px-3 py-3">
                    ...
                    <p className="mt-0.5 max-w-[200px] text-[10px] text-subtle">
                      {option.recommendedFor}
                    </p>
                  </td>
                  <td className="px-3 py-3 whitespace-nowrap text-muted-fg">
                    {option.startDate.slice(5)} → {option.endDate.slice(5)}
                  </td>
                  <td className="px-3 py-3 text-fg">{option.gameCount}</td>
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-subtle">{option.recommendedFor}</p>
                </div>
              ...
              <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted-fg">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 text-[11px]">
                <div>
                  <p className="mb-1 font-medium uppercase tracking-wide text-subtle">
                    Days per city
                  </p>
                  <ul className="space-y-0.5 text-fg-soft">
                    {analysis.daysPerCity.map(({ city, days }) => (
                      <li key={city}>
